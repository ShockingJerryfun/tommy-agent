...
from app.agent_framework.tools import ToolRegistry
...
def test_dangerous_shell_command_is_outright_denied():
    policy = default_permission_policy()
    decision = policy.evaluate(
        "run_shell_command",
        {"command": "sudo rm -rf /"},
...
def test_unrestricted_scope_skips_approval_but_still_blocks_dangerous():
    policy = default_permission_policy()
    ...
    assert dangerous.denied is True
...
def test_dangerous_shell_returns_permission_denied_error():
    runtime = ToolRuntime(_registry())
