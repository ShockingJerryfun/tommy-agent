...
def test_replay_session_uses_persisted_user_inputs() -> None:
    ...
    def runner(user_input: str, _ctx):
        ...
        return {
            "final_response": (f"answer: {user_input} https://example.com/{len(seen)}"),
...
def test_replay_run_events_reads_sequence_without_model_calls() -> None:
    store = _store()
    sid = _new_session(store)
    run = store.create_run(session_id=sid, agent_id="default", input="hello")
    rid = str(run["id"])
    store.append_run_event(sid, run_id=rid, type="model_start", label="模型开始", payload={})
    ...
    store.append_run_event(sid, run_id=rid, type="done", label="完成", payload={})

    def forbidden_runner(*_args, **_kwargs):
        raise AssertionError("replay_run_events must not invoke models")

    report = replay_run_events(store, run_id=rid, runner=forbidden_runner)

    assert report.run_id == rid
    assert report.event_count == 3
    assert report.event_types == ["model_start", "message_delta", "done"]
    assert report.reconstructed_text == "Hello"
    assert [event["sequence"] for event in report.events] == [0, 1, 2]
...
def test_skill_forge_maintenance_is_opt_in_by_default(monkeypatch) -> None:
    monkeypatch.delenv("TOMMY_SKILL_FORGE_ENABLED", raising=False)
    jobs = default_maintenance_jobs(_store())

    assert "skills.forge_nightly" not in {job.name for job in jobs}


def test_skill_forge_maintenance_can_be_enabled(monkeypatch) -> None:
    monkeypatch.setenv("TOMMY_SKILL_FORGE_ENABLED", "1")
    jobs = default_maintenance_jobs(_store())
