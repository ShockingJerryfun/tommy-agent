...
from app.agent_framework.tools import ToolRegistry, get_current_time
...
def test_registry_orders_hooks_by_order_then_seq() -> None:
    ...
    registry.register(
        name="b", phase=HookPhase.RUN_START,
        callable=lambda _ctx: log.append("b"), order=10,
    )
    registry.register(
        name="a", phase=HookPhase.RUN_START,
        callable=lambda _ctx: log.append("a"), order=20,
    )
    registry.register(
        name="c", phase=HookPhase.RUN_START,
        callable=lambda _ctx: log.append("c"), order=10,
    )
