    ...
    *,
    ...
    for content, importance in seeds:
        ...
        if embedder is not None:
            ...
            if vec:
                store.memories.update_embedding(row["id"], embedding=vec, model=embedder.model)
        rows.append(row)
    return store, rows
...
def test_fts_only_retrieval_returns_relevant_rows() -> None:
    ...
    retriever = HybridRetriever(store, embedder=NullEmbedder(), reranker=NullReranker())
...
def test_vector_retrieval_with_echo_embedder() -> None:
    ...
    retriever = HybridRetriever(store, embedder=embedder, reranker=NullReranker(), fts_limit=0)
...
def test_rrf_fusion_combines_both_rankers() -> None:
    ...
    has_both = any(c.fts_rank is not None and c.vector_score is not None for c in result.candidates)
...
def test_reranker_swaps_top_candidate() -> None:
    store, _ = _make_store_with_active_memory()
    retriever = HybridRetriever(store, embedder=NullEmbedder(), reranker=_PrefixReranker())
...
def test_forgetter_decays_low_importance_unused_memory() -> None:
    ...
    listed = {row["id"]: row for row in store.list_memories(agent_id="default", limit=20)}
...
def test_provider_retrieve_for_context_shape() -> None:
    ...
    candidates = provider.retrieve_for_context("Fang Jin", agent_id="default", top_k=3)
    assert candidates
    for cand in candidates:
