...
def test_forge_propose_reuses_existing_pending_chain_proposal():
    store = _store()
    forge = SkillForge(store=store, embedder=EchoEmbedder())

    first = forge.propose(agent_id="default", chain=("web_search", "context_pact_update"))
    second = forge.propose(agent_id="default", chain=("web_search", "context_pact_update"))

    assert second["proposal"]["id"] == first["proposal"]["id"]
    assert second["skill"]["id"] == first["skill"]["id"]
    assert len(store.list_skill_proposals(agent_id="default", status="proposed")) == 1
